...
from .getkw import GetkwParser, Section
...
def parse_pcm_input(inputFile, write_out=False):
    ...
    if write_out:
        ...
        parsedFile = os.path.join(
            os.path.dirname(inputFile), "@" + os.path.basename(inputFile)
        )
...
def setup_keywords():
    ...
    # Valid values: VACUUM, UNIFORMDIELECTRIC, SPHERICALSHARP
    # Default: VACUUM
    ...
    # Center of the sharp profile
    # Valid for: SPHERICALSHARP
    # Valid values: positive double
    # Default: 100.0
    # Notes: for SPHERICALSHARP corresponds to the sphere radius
    ...
    charge_distribution = Section(
        "CHARGEDISTRIBUTION", callback=verify_charge_distribution
    )
    # Monopoles
    # Valid values: array of doubles in format [x, y, z, Q]
    # Notes: charges are always in atomic units
...
def verify_top(section):
    ...
    if val not in allowed_units:
        print(
            (
                "Units requested {} are not among the allowed units: {}".format(
                    val, allowed_units
                )
            )
        )
    ...
    if CODATAyear not in allowed_codata:
        print(
            (
                "CODATA set requested {} is not among the allowed sets: {}".format(
                    CODATAyear, allowed_codata
                )
            )
        )
        sys.exit(1)
...
def verify_cavity(section):
    ...
    if type.get() not in allowed:
        print(
            (
                "Requested {} cavity is not among the allowed types: {}".format(
                    type, allowed
                )
            )
        )
    ...
    if type.get() == "GEPOL":
        ...
        if a < 0.01:
            print(
                (
                    "Requested area value too small: {}. Minimal value is: 0.01 au^2".format(
                        a
                    )
                )
            )
        ...
        if mr < 0.4:
            print(
                (
                    "Requested minimal radius for added spheres too small: {}. Minimal value is: 0.4 au".format(
                        mr
                    )
                )
            )
    ...
    if radiiSet.get() not in allowed_sets:
        print(
            (
                "Radii set requested {} is not among the allowed sets: {}".format(
                    radiiSet.get(), allowed_sets
                )
            )
        )
        sys.exit(1)
    ...
    if mode.get() not in allowed_modes:
        print(
            (
                "Cavity creation mode requested {} is not among the allowed modes: {}".format(
    ...
    if mode.get() == "ATOMS":
        if len(r) != len(at) or len(at) == 0:
            print(
                "Incoherent input for Atoms keyword. Check that Atoms and Radii are consistent."
            )
            sys.exit(1)
        else:
            for i, v in enumerate(at):
                if at.count(v) > 1:
                    print(
                        "Incoherent input for Atoms keyword. Too many spheres on the same atom(s)."
                    )
                    sys.exit(1)
...
def convert_area_scalar(keyword):
    if isAngstrom:
        keyword[0] /= (
            CODATAdict[CODATAyear].ToAngstrom * CODATAdict[CODATAyear].ToAngstrom
        )

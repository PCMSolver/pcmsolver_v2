...
def collect_version_input_from_git():
    ...
    if mobj.group("tag"):
        ...
        res["latest_annotated_v_tag"] = mobj.group("tag")[
            :-1
        ]  # drop the "v"; tag mismatch caught later
...
def reconcile_and_compute_version_output(quiet=False):
    ...
    if is_git_repo(cwd=cwd) and not res["is_hard_copy"]:
        ...
        if res["latest_annotated_v_tag"] == meta_latest_annotated_v_tag:
            ...
            trial_version_long_release = (
                res["latest_annotated_v_tag"] + "+" + res["seven_char_hash"]
            )
            trial_version_devel = (
                res["upcoming_annotated_v_tag"] + ".dev" + res["commits_since_tag"]
            )
            trial_version_long_devel = (
                trial_version_devel + "+" + res["seven_char_hash"]
            )
            ...
            if int(res["commits_since_tag"]) == 0:
                ...
                if trial_version_long_release == res["long"]:
                    ...
                    if meta_seven_char_hash == "zzzzzzz":
                        if not quiet:
                            print(
                                """Defining {} version: {} (recorded and computed)""".format(
                                    "prerelease" if mobj.group("prere") else "release",
                                    trial_version_long_release,
                                )
                            )
                ...
                if res["branch_name"].endswith(".x"):
                    print(
                        """Undefining version as development snapshots not allowed on maintenance branch: {} (rejected computed)""".format(
                            trial_version_long_devel
                        )
                    )
                ...
                # TODO prob should be undef unless on master
                    ...
                    if not quiet:
                        print(
                            """Defining development snapshot version: {} (computed)""".format(
                                trial_version_long_devel
                            )
                        )
            ...
            print(
                """Undefining version for irreconcilable tags: {} (computed) vs {} (recorded)""".format(
        ...
        print(
            """Blindly (no git) accepting release version: {} (recorded)""".format(
                res["long"]
            )
        )
...
def version_formatter(versdata, formatstring="""{version}"""):
    ...
    release = (
        "release"
        if versdata["__version_release"]
        else ("prerelease" if versdata["__version_prerelease"] else "")
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Script to extract PCMSolver version from source. Use pcmsolver.version_formatter(fmt_string) after build."
    )
    parser.add_argument(
        "--metaout",
        default="metadata.out.py",
        help="file to which the computed version info written",
    )
    parser.add_argument(
        "--cmakeout",
    ...
    parser.add_argument(
        "--headerout",
        default="metadata.out.h",
        help="header file to which the version info written",
    )
    parser.add_argument(
        "--format",
        default="all",
